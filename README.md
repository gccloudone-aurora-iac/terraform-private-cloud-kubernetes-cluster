...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_additional_pools"></a> [additional\_pools](#input\_additional\_pools) | List of pools for the cluster. | <pre>list(object({<br/>    name    = string<br/>    roles   = optional(list(string), ["worker"])<br/>    flavour = optional(string, "g4v-16")<br/>    count   = optional(number, 1)<br/>    labels  = optional(map(string))<br/>    taints = optional(list(object({<br/>      key    = string<br/>      value  = string<br/>      effect = optional(string, "NoExecute")<br/>    })), [])<br/>    security_groups = optional(list(string), ["default"])<br/>    volume_type     = optional(string, "performance")<br/>    volume_size     = optional(number, 60)<br/>    image_name      = optional(string)<br/>    user_data       = optional(string)<br/>    network_id      = optional(string)<br/>  }))</pre> | `[]` | no |
| <a name="input_application_credential_id"></a> [application\_credential\_id](#input\_application\_credential\_id) | Application Credential ID for authentication with OpenStack. | `any` | n/a | yes |
| <a name="input_application_credential_secret"></a> [application\_credential\_secret](#input\_application\_credential\_secret) | Application Credential Secret for authentication with OpenStack. | `any` | n/a | yes |
| <a name="input_auth_url"></a> [auth\_url](#input\_auth\_url) | OpenStack Authentication URL | `any` | n/a | yes |
| <a name="input_availability_zone"></a> [availability\_zone](#input\_availability\_zone) | OpenStack Availability Zone where to deploy the cluster. | `any` | `null` | no |
...
| <a name="input_domain_id"></a> [domain\_id](#input\_domain\_id) | Domain ID of the OpenStack tenant where to deploy the cluster.. | `any` | n/a | yes |
...
| <a name="input_project_id"></a> [project\_id](#input\_project\_id) | Project ID of the OpenStack project where to deploy the cluster. | `any` | n/a | yes |
| <a name="input_region"></a> [region](#input\_region) | OpenStack Region where to deploy the cluster. | `any` | n/a | yes |
